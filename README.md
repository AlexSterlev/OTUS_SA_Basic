...
#### Критерии приёмки: ####
1. Менеджер может открыть список свободных мастеров через интерфейс управления заказами/нарядами.
2. В списке отображаются только те мастера, которые в данный момент не заняты другими задачами.
3. Список позволяет фильтровать и сортировать мастеров по различным критериям.
4. Для каждого мастера в списке отображается вся необходимая информация.
5. Менеджер может выбрать мастера из списка и назначить его на задачу.
6. После назначения мастера на задачу, он исчезает из списка свободных мастеров (если у него не осталось свободного времени).
7. Информация о занятости мастеров обновляется в реальном времени или через короткие интервалы.
#### Технические детали: ####
- Backend:
Создать API endpoint для получения списка свободных мастеров.
Endpoint должен учитывать текущие задачи мастеров и их расписание.
- Frontend:
Создать компонент для отображения списка свободных мастеров.
Обеспечить возможность фильтрации и сортировки списка.
Обеспечить возможность назначения мастера на задачу через интерфейс.
