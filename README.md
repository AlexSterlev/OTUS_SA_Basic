...
| US004 | Я как менеджер хочу передать ТС мастеру для проведения диагностики | must | Менеджер | mvp | Менеджер может выбрать техническое средство (ТС) для передачи, указать мастера, которому будет передано ТС.<br><br>Система должна уведомить мастера о передаче ТС для проведения диагностики. |
...
| US008 | Я как менеджер хочу проверить наличие запчастей для согласования сроков выполнения заказ-наряда | must | Менеджер | mvp | Менеджер может увидеть в системе список запчастей, необходимых для выполнения заказ-наряда. |
...
| US015 | Я как менеджер хочу иметь форму заказа запчастей для составления заявки на склад | should | Менеджер |     | Менеджер имеет доступ к форме заказа запчастей через соответствующий интерфейс |
